...
export class ItemService {
    ...
    return this.http.get('http://wah.jonaskf.net/GetItems.php?itemid=' + itemid)
      .map(response => <Object>function (r) {
        if (r.itemID !== undefined) {
          try {
            Database.db.table('items').put(r).catch(c => {
              console.log(c);
            });
          } catch (err) {
  ...
  getItems(): Promise<any> {
    ...
    return this.httpClient.get(chooseUrl(apiUrl, localUrl))
      .map(response => <Object>function (r) {
        Database.db['items'].clear();
        Database.db['items'].bulkAdd(r);
  ...
  getPets(): Promise<any> {
    ...
    return this.httpClient.get(chooseUrl(apiUrl, localUrl))
      .map(response => <Object>function (r) {
        ...
        Database.db.table('pets').bulkPut(r);
        return r;
  ...
  getRecipes(): Promise<any> {
    ...
    return this.httpClient.get(
      chooseUrl(apiUrl, localUrl)).toPromise();
  }
